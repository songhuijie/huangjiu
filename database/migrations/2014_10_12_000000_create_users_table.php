...
//        });
        Schema::create('user', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->integer('parent')->default(0)->comment('上一级ID 此ID为上一级推荐 下一级不为下一级 推荐上一级');
            $table->string('user_nickname')->comment('用户昵称');
            $table->string('user_img')->comment('用户头像');
            $table->float('user_balance')->default(0)->comment('用户可提现余额');
            ...
            $table->integer('user_type')->default(1)->comment('1普通会员 2合伙人 3站长');
