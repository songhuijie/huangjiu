...
<body>
    ...
    layui.use(['util','form','laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element', 'slider'], function(){
        ...
        var insTb = table.render({
            elem: '#demo'
            ,height: 800
            ,url: '{{URL("label/index")}}?type=select' //数据接口
            ,title: '标签表'
            ,page: true //开启分页
            ...
            switch(obj.event){
              case 'add':
                layer.open({
                  ...
                  type: 2, 
                  area: ['500px', '400px'],
                  content: '{{url("label/detail")}}' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
                }); 
                //layer.msg('添加');
              break;
              case 'update':
                if(data.length === 0){
                  ...
                  layer.open({
                      ...
                      type: 2, 
                      area: ['500px', '400px'],
                      content: '{{url("label/detail")}}?type=edit&id='+data[0].id
