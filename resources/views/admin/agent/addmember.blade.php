...
<form class="form-horizontal" id="horizontal" >
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="text" name="name" class="form-control" id="inputEmail3" placeholder="姓名"  value=" @if (!empty($data)) {{$data['name']}}  @endif ">
    </div>
  </div>
    ...
    <label for="inputEmail3" class="col-sm-2 control-label">性别</label>
        @if (!empty($data))
            ...
            <label class="radio-inline">
              <input type="radio" name="sex" @if ($data['sex'] == 1) checked  @endif id="inlineRadio1" value="1"> 男
            </label>
            <label class="radio-inline">
              <input type="radio" name="sex" @if ($data['sex'] == 0) checked  @endif  id="inlineRadio2" value="0"> 女
            </label>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-10">
      <input type="text" name="age" class="form-control" id="inputEmail3" placeholder="年龄" value=" @if (!empty($data)) {{$data['age']}}  @endif">
    </div>
  </div>

  <div class="form-group">
    <label for="inputEmail3" class="col-sm-2 control-label">所在团队</label>
    @if(!empty($data)) 
        <div class="col-sm-10">
          <input type="text"  readonly class="form-control" id="inputEmail3" placeholder="所在团队" value=" {{$data['teamname']}}">
    ...
    <select class="form-control" name="teamid">
      ...
      @foreach ($list as $key=>$datas )
        @if (empty($data)) 
            <option value="{{$datas['id']}}" >{{$datas['teamname']}}</option>
        @else

            @if ($datas['id'] == $data['teamid']  )
                <option value="{{$datas['id']}}" selected>{{$datas['teamname']}}</option>
            @else
                <option value="{{$datas['id']}}" >{{$datas['teamname']}}</option>
            @endif
        @endif
...
var radios = document.getElementById("sheng");//获取id为list下的所有name为user的值的集合
          if(radios.checked){
            $("#pid").hide();
          }
