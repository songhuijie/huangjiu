...
class AgentController extends Controller
{
    ...
    private $order;
    private $income;
    public function __construct(Agent $agent,AgentSet $agent_set,Config $config,Friend $friend,FriendShip $friend_ship,User  $user,Order $order,IncomeDetails $income)
    {
        $this->agent = $agent;
    ...
    public function SubordinateUser(){
        ...
        if($agent && $agent->status == 1){
            $status = 1;
            $result = $this->friend_ship->getByBest($user_id);
        }else{
            $init_friend = $this->friend_ship->getByUser($user_id);
            if($init_friend && $init_friend->status != 0){
                $status = 2;
                $result = $this->friend_ship->ShipQuery($user_id);
            }else{
                $status = 0;
                $result = [];
            }
    ...
    public function setAgent(Request $request){
        ...
        if($agent){
            ...
            $friend_ship = $this->friend_ship->getByUser($set_user_id);
            $thing3 = '';

            if($friend_ship){
                switch ($type){
                    case 1:
                        $this->friend_ship->updateAgent($set_user_id,$user_id,$type);
                        $response_json->status = Lib_const_status::SUCCESS;
                        $thing3 = '发货人员';
                        break;
                    case 2:
...
//            $friend_ship = $this->friend_ship->getByUser($user_id);
//            if($friend_ship && $friend_ship->status != 0){
//
//            }
        }
        return $this->response($response_json);
    ...
    public function cancelAgent(Request $request){
        ...
        $type = isset($all['type'])?$all['type']:1;
        $friend = $this->friend_ship->getByUser($set_user_id);
        if($friend){
            $this->friend_ship->updateAllAgentByID($set_user_id,$user_id);
            $response_json->status = Lib_const_status::SUCCESS;
        }else{
